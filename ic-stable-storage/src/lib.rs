...
pub(crate) use stable_structures::btreemap::InsertError;
...
#[cfg(test)]
mod test {
    use super::{VirtualMemory, export::stable_structures::RestrictedMemory, *};
    use std::rc::Rc;
  ...
  fn multiple_entry_grow_size() {
      ...
      let virtual_memory_0 =
          VirtualMemory::<_, 0>::init(Rc::clone(&data_memory));
      let virtual_memory_1 =
          VirtualMemory::<_, 1>::init(Rc::clone(&data_memory));
  ...
  fn write_multiple_entries_spanning_multiple_pages() {
      ...
      let virtual_memory_2 =
          VirtualMemory::<_, 2>::init(Rc::clone(&data_memory));
