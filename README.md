...
canister, because all the API's of that dependency will not be included into the canister you're implementing. If you create a canister using `canister-sdk`, don't forget to add a `export-api` feature to the dependency so that it will export all of the wasm from the canister trait definition (more specifically from one that is auto-generated by `generate_exports!` macro).
